document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    pacientes.forEach((paciente) => {
      ...
      row.setAttribute("data-paciente-cedula", cedula);
    ...
    editIcons.forEach((editIcon) => {
      editIcon.addEventListener("click", (event) => {
        ...
        const cedula = row.getAttribute("data-paciente-cedula");
        redirigirAFormulario(cedula);
      });
    });
  } catch (error) {
    console.error(error);
...
function redirigirAFormulario(cedula) {
  window.location.href = `actualizarPaciente.html?cedula=${cedula}`;
}
